...
const Cover = ({ url, preview }: CoverImageProps) => {
  ...
  // OPEN FILE MODAL TO CHANGE IMAGE
  ...
  const onRemove = async () => {
    // Also delete the image from edge store cloud
    if(url) {
  ...
  return (
    <div className={cn("relative w-full h-[35vh] group", !url && "h-[12vh]", url && "bg-muted")}>
      ...
      {url && !preview && (
        <div className="opacity-0 group-hover:opacity-100 absolute bottom-5 right-5 flex items-center gap-x-5">
